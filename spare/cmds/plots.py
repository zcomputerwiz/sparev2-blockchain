...
def show_plots(root_path: Path):
    from spare.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'spare plots add -d [dir]' and remove with"
        + " 'spare plots remove -d [dir]'"
        + " Scan and check plots with 'spare plots check'"
    )
    print()
...
def create_cmd(
    ...
    from spare.plotting.create_plots import create_plots, resolve_plot_keys
...
def check_cmd(
    ...
    from spare.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from spare.plotting.util import remove_plot_directory
