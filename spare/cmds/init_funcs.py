...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("spare_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("spare_target_address"):
    ...
    if "spare_target_address" not in config["farmer"]:
        print(
            f"Setting the spare destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
