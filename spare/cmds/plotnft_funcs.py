...
from spare.cmds.wallet_funcs import print_balance, wallet_coin_unit
from spare.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
...
from spare.server.server import ssl_context_for_root
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_gravitons = uint64(int(fee * units["spare"]))
...
async def self_pool(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    func = functools.partial(wallet_client.pw_self_pool, wallet_id, fee_gravitons)
    await submit_tx_with_confirmation(msg, prompt, func, wallet_client, fingerprint, wallet_id)
