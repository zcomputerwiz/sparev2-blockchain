...
from spare.consensus.network_type import NetworkType
...
from spare.protocols.harvester_protocol import PoolDifficulty
...
class FarmerAPI:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"Spare Blockchain v.{__version__}"},
                        ) as resp:
                            if resp.ok:
    ...
    @api_request
    @peer_required
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: ws.WSSpareConnection):
