...
from spare.wallet.puzzles.puzzle_utils import (
...
from spare.wallet.secret_key_store import SecretKeyStore
from spare.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_spare(self, spare_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if spare_amount < 0:
            utxos = await self.select_coins(abs(spare_amount), exclude)
        else:
        ...
        spare_amount = spend_value + spare_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": spare_amount}]
