...
ROOT = pathlib.Path(importlib.import_module("spare").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `spare` file name (which is the executable)
    and the `spare` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `spare` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("spare/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("spare-blockchain"))[0]
...
# TODO: collapse all these entry points into one `spare_exec` entrypoint that accepts the server as a parameter

entry_points = ["spare.cmds.spare"] + [f"spare.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "spare.util.bip39"])

if THIS_IS_WINDOWS:
    spare_mod = importlib.import_module("spare")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/spare/util/english.txt", "spare/util"))
datas.append((f"{ROOT}/spare/util/initial-config.yaml", "spare/util"))
datas.append((f"{ROOT}/spare/wallet/puzzles/*.hex", "spare/wallet/puzzles"))
datas.append((f"{ROOT}/spare/ssl/*", "spare/ssl"))
...
add_binary("spare", f"{ROOT}/spare/cmds/spare.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/spare/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/spare/server/start_{server}.py", COLLECT_ARGS)
