...
def show_plots(root_path: Path):
    ...
    print(
        "Add with 'replaceme plots add -d [dir]' and remove with"
        + " 'replaceme plots remove -d [dir]'"
        + " Scan and check plots with 'replaceme plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'replaceme init'")
...
def create_cmd(
    ...
    from replaceme.plotting.create_plots import create_plots, resolve_plot_keys
...
def check_cmd(
    ...
    from replaceme.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from replaceme.plotting.util import remove_plot_directory
