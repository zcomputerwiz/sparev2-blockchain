...
import stat
import sys
from replaceme.util.config import load_config, traverse_dict
from replaceme.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `replaceme init --fix-ssl-permissions` to fix issues.")
