...
if [ "$(id -u)" = 0 ]; then
  echo "The Spare Blockchain GUI can not be installed or run by the root user."
	exit 1
fi

# Allows overriding the branch or commit to build in spare-blockchain-gui
SUBMODULE_BRANCH=$1
...
echo ""
echo "Spare blockchain install-gui.sh completed."
echo ""
echo "Type 'cd spare-blockchain-gui' and then 'npm run electron &' to start the GUI."
