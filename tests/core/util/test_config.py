...
from spare.util.config import create_default_spare_config, initial_config_file, load_config, save_config
...
class TestConfig:
    ...
    def test_create_config_new(self, tmpdir):
        """
        Test create_default_spare_config() as in a first run scenario
        """
    ...
    def test_create_config_overwrite(self, tmpdir):
        """
        Test create_default_spare_config() when overwriting an existing config.yaml
        """
        # When: using a clean directory
        root_path: Path = Path(tmpdir)
        config_file_path: Path = root_path / "config" / "config.yaml"
